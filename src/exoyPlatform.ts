...
import { Light, LightInstance } from './light';

export class ExoyPlatform implements DynamicPlatformPlugin {
		...
		this.api.on('didFinishLaunching', () => {
			...
			new Bonjour().find(
				...
				(remoteService: BonjourService) => {
					if (!remoteService.name.startsWith('exoyone')) return;
					...
					if (this.lights.has(light.mac)) {
						// there already is a platform handler active for this Light, so just update the connection data
						this.log.debug(
							'Updating connection data for accessory:',
							remoteService.name,
						);
						...
						this.log.info(
							'Discovered accessory on network:',
							remoteService.name,
						);

						LightInstance.createInstance(
							light,
							this.log,
	 ...
	 */
	configureDevice(light: LightInstance) {
		// generate a unique id for the accessory from the serial number
		const uuid = this.api.hap.uuid.generate(light.info!.mdnsName);
		...
		if (accessory) {
			...
			// update the context
			accessory.context.device = light satisfies Light;
			this.api.updatePlatformAccessories([accessory]);

			// create the accessory handler for the restored accessory
			this.lights.set(
				light.mac,
				new KeyLightsAccessory(this, accessory, light),
			);
		} else {
			// the accessory does not yet exist, so we need to create it
			this.log.info(
				'Adding new accessory to Homebridge:',
				light.name,
				'as',
				light.displayName,
			);
